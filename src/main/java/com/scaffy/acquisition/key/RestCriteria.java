...
public class RestCriteria {
	...
	public BigDecimal getDecimalValue() throws BadCriteriaValueException{
		
		String encodedValue = getValue();
		
		if(encodedValue.matches(INTEGER_PATTERN)){
			
			try{
				
				return new BigDecimal(Integer.parseInt(encodedValue));
				
			} catch(NumberFormatException e){
				
				return new BigDecimal(Long.parseLong(encodedValue));
			}
		}

		else if(encodedValue.matches(DOUBLE_Pattern))
			return new BigDecimal(Double.parseDouble(encodedValue));
